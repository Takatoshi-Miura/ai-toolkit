# retrospective-claude-usage-daily コマンド用タスク

## 概要
Claude Codeの前日までのセッション履歴を分析し、活動内容をレポート形式で整理する。
セッション履歴は`~/.claude/history.jsonl`から取得する。
...
### 期間指定オプション
- `--since YYYY-MM-DD`: 指定日以降
- `--until YYYY-MM-DD`: 指定日まで
- `--last-7days`: 過去7日間
