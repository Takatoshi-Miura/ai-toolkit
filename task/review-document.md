# review-document コマンド用タスク

## 概要
Google Drive資料またはGitHub PRをレビューする。URLの形式に応じて適切な処理を実行する。
...
2. URLの形式を判別する
    - `github.com` を含む場合 → **PRレビューフロー**（Phase 2A）へ
    - `docs.google.com` または `drive.google.com` を含む場合 → **資料レビューフロー**（Phase 2B）へ

---

### Phase 2A: PRレビューフロー（GitHub PR の場合）

3A. 追加情報を質問して提供してもらう（任意）
    - PBIのURL
    - 概要設計書のURL
    - テスト項目書のURL

4A. PRレビュー対象のブランチにチェックアウトされているか確認し、されていなければユーザーに要求する

5A. PRをmcp-github__get_pull_request で読み取る

6A. 3Aで追加情報が提供されていれば、mcpを使用して情報を読み取る
...
7A. 関連するプロジェクトのコードを読み取り、PRの変更差分(diff)を確認し、変更されたファイルと行のみをレビュー対象とする

8A. 「PRレビュー観点」に従ってレビューを実施し、結果をユーザーに共有する

---

### Phase 2B: 資料レビューフロー（Google Drive の場合）

3B. 追加情報を質問して提供してもらう（任意）
    - 要求事項（例: 「技術的な正確性を確認してほしい」「初心者向けに分かりやすいか確認してほしい」など）
    - 読み込みたいシート名またはドキュメントタブ名

4B. ~/Documents/Git/ai-toolkit/task/read-google-drive.md の手順に従い、資料を読み取る

5B. 資料の内容を理解し、以下のレポートを作成する
    - 要求事項がある場合: 要求事項に沿ったレポートを作成
    - 要求事項がない場合: 「資料レビューテンプレート」に従ってレポートを作成

6B. レポートをユーザーに提示し、追加の質問や深掘りしたい点がないか確認する
