...
- **システム通知チャンネル**: 運用通知の送信先チャンネルIDを設定
- **メンション対象ユーザー**: 必要に応じてユーザーIDを制限
- **ルーティングルール**: スキル名・通知先チャンネルID・キーワードを設定
...
1. **ルーティングルール**テーブルに新しい行を追加（スキル名・通知先チャンネルID・キーワード）
2. リスナーを再起動（CONFIG.mdは起動時に読み込まれるため）
