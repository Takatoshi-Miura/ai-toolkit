...
GitHub Pull Requestをレビューする際の観点と手順を定義します。
...
5. **変更範囲の確認**
   - 関連するプロジェクトのコードを読み取り
   - PRの変更差分(diff)を確認
   - 変更されたファイルと行のみをレビュー対象とする

6. **レビュー観点チェック**
   - 下記「PRレビュー観点」に従ってレビューを実施
   - 結果をユーザーに共有
...
### 3. プラットフォーム固有の考慮事項
* プラットフォーム（Web/iOS/Android等）のベストプラクティスに沿っている
* プラットフォーム固有のAPIやライフサイクルを正しく扱っている
* リソース管理（メモリ、ファイルハンドル等）が適切
