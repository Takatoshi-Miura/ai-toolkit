...
SportsNote Androidのコードを調査し、`doc/spec/`の仕様書とプロジェクトルール（`CLAUDE.md`）を実装の実体に合わせて更新する。承認は不要で、確定情報として直接更新する。
...
`doc/spec/00_overview.md`が存在しない場合（初回実行時）、先に`ui/`配下のサブディレクトリ一覧（`group/`, `note/`, `task/`, `target/`, `measures/`, `setting/`等）から画面一覧を洗い出し、`doc/spec/00_overview.md`（画面一覧・画面遷移図）を作成する。作成後、通常の調査範囲決定に進む（このPhase内で完結させ、Phase 2はスキップしない）。

`references/knowledge-map.md`を読み、前回スキャン済みの範囲を確認する。1回の実行につき調査対象を1〜2ディレクトリ程度に絞る（`model/`, `viewModel/`, `ui/<サブディレクトリ>`, `model/manager/`等をローテーションする）。全件走査はコンテキスト消費が大きいため行わない。

`git checkout main && git pull`でmainを最新化し、`git checkout -b knowledge-update-<日付>`で作業ブランチを作成する。

**成功確認**: 今回の調査対象ディレクトリが確定し、作業ブランチを作成した（初回は00_overview.md作成も完了した） → Phase 2へ
...
| CLAUDE.mdと実体の乖離 | ビルドコマンドのパス、ディレクトリ構成の記載、`versionCode`/`versionName`/`targetSdk`等アプリ設定値、テストディレクトリの実在確認等 |
...
Phase 2で見つかった差分のうち、**開発運用ルールに関わるもの**（ビルドコマンドのパス誤り、コーディング規約の実体との乖離、アプリ設定値の陳腐化等）のみを対象に`CLAUDE.md`を更新する。仕様レベルの乖離はPhase 3のdoc更新で吸収し、CLAUDE.mdの編集範囲は運用ルールに厳格に絞る。
...
- 今回の調査対象ディレクトリ（初回は00_overview.md作成も明記）
...
| エラー | 対応 |
|-------|------|
| `doc/spec/`が見つからない（00_overview.md作成前） | Phase 1の初回セットアップ分岐で新規作成する。それでも`ui/`配下から画面一覧を洗い出せない場合はhistory.mdに記録して終了 |
