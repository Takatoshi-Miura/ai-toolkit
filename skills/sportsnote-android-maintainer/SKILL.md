---
name: sportsnote-android-maintainer
description: SportsNote Android（/Users/it6210/Documents/Git/SportsNote_Android）の開発・保守を無人実行するオーケストレーター。コード調査によるdoc/仕様書更新（ナレッジ蓄積）、不具合調査・ユーザー要望・リファクタリング候補・ドキュメント不足・テスト不足の観点からのissue起票（issue作成）、issueの選択から実装・クロスレビュー・PR作成まで（実装）の3フローを提供する。ローカルルーチンからの「ナレッジ蓄積フロー」「issue作成フロー」「実装フロー」の呼び出しで使用。
allowed-tools: Read, Write, Edit, Bash, Agent, TodoWrite, Glob, Grep
user-invocable: true
disable-model-invocation: true
---

# sportsnote-android-maintainer

SportsNote Androidの開発・保守に関する3つのフローを提供する。最初にどのフローを行うかを判定し、対応するWORKFLOWファイルの手順に従うこと。
...
- **他スキルとの関係**: `sportsnote-ios-maintainer`（iOS版）とは完全に独立している。ファイルの参照・importは行わない

## 対象リポジトリ

- **プロジェクトパス**: `/Users/it6210/Documents/Git/SportsNote_Android`
- **プロジェクト規約**: `/Users/it6210/Documents/Git/SportsNote_Android/CLAUDE.md`
- **仕様書ディレクトリ**: `/Users/it6210/Documents/Git/SportsNote_Android/doc/spec/`（初回実行時はディレクトリ自体が存在しないため、WORKFLOW-KNOWLEDGE.mdのPhase 1で自動作成する）
- **GitHubリポジトリ**: `Takatoshi-Miura/SportsNote_Android`（`gh` CLIでログイン済み前提）
...
| SportsNote_Androidリポジトリが存在しない | パスを確認し、history.mdに記録して終了 |
