...
`gh pr list --repo Takatoshi-Miura/SportsNote_Android --state open --json number,title,headRefName,mergeable`で全openPRを取得する。
...
5. `./gradlew ktlintFormat`実行 → `./gradlew assembleDebug`実行。失敗した場合はエラーを分析し修正、**最大3回まで**自己修正を試みる
...
`gh issue list --repo Takatoshi-Miura/SportsNote_Android --state open --json number,title,labels,createdAt`で全openissueを取得する。
...
prompt: |
  ...
  ## 参照情報
  - CLAUDE.md: /Users/it6210/Documents/Git/SportsNote_Android/CLAUDE.md
  - 仕様書: /Users/it6210/Documents/Git/SportsNote_Android/doc/spec/（関連ファイルを確認）

  ## 作成してほしいもの
  1. issue本文の「対応方針」を踏まえた具体的な実装ステップ
  2. 既存の類似実装パターン調査（SportsNote_Android内）
...
4. `./gradlew ktlintFormat`実行
5. `./gradlew assembleDebug`実行。失敗した場合はエラーを分析し修正、**最大3回まで**自己修正を試みる
6. issue本文の「完了条件」チェックリストの各項目を確認する。変更対象に対応する既存テスト（`app/src/test/`配下）がある場合は`./gradlew test`を実行し成功させる。テスト不足issue（`test`ラベル）の対応の場合は新規テストを追加し実行・成功させる。それ以外で対象に既存テストが存在しない場合、テスト実行は完了条件の必須項目にしない
...
**完了条件（すべて満たすこと）**:
- [ ] `./gradlew ktlintFormat`が正常に完了
- [ ] `./gradlew assembleDebug`が成功
- [ ] 既存テストまたは新規追加テストがある場合は`./gradlew test`が成功
