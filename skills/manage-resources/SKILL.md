...
以下のJSON形式でAskUserQuestionツールを呼び出す:
  ...
  "questions": [
    {
      ...
      "options": [
        {
          "label": "新規作成",
          "description": "新しいスラッシュコマンド、サブエージェント、Skillを作成します"
        },
        {
          "label": "更新",
          "description": "既存リソースを修正・改善します"
        },
        {
          "label": "メンテナンス",
          "description": "リポジトリ全体の整合性チェック・リファクタリングを実施します"
        },
        {
          "label": "同期",
          "description": "ai-toolkitの変更を~/.claude/に一方向同期します"
        }
      ],
      "multiSelect": false
    }
...
**必須**: ユーザーの回答が上記4つの選択肢のいずれかに明確に該当するか検証する。
- 回答が空、不明瞭、または選択肢に該当しない場合 → **0-1のAskUserQuestionを再度呼び出す。デフォルト値で進めてはならない。**

回答に応じて、該当するワークフローファイルを参照し実行する:

- **「新規作成」を選択** → [WORKFLOW-CREATE.md](WORKFLOW-CREATE.md) を参照して実行
- **「更新」を選択** → [WORKFLOW-UPDATE.md](WORKFLOW-UPDATE.md) を参照して実行
- **「メンテナンス」を選択** → [WORKFLOW-MAINTAIN.md](WORKFLOW-MAINTAIN.md) を参照して実行
- **「同期」を選択** → [WORKFLOW-SYNC.md](WORKFLOW-SYNC.md) を参照して実行
