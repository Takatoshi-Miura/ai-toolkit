# cloud-am6-line-recommender セットアップ
...
3. Instructions（実行内容）欄に以下を記載する：
   ...
   ```
4. スケジュールを「毎日 6:00（ローカルタイムゾーン）」に設定する
...
| `MYHOME_CONDITIONS` | 調査条件（JSON文字列）。キーの例: 物件種別・購入期限・エリア・通勤条件・予算・築年数・間取り・駐車場・ペット可否・騒音や水害や虫の少なさ・コンビニ/スーパーの徒歩圏内・避けたいもの・補足。値はClaudeデスクトップアプリの環境変数設定画面で直接入力する（このファイルには具体値を記載しない） |
...
- 調査条件を変更したい場合は、references/CONFIG.md ではなく環境変数 `MYHOME_CONDITIONS` を更新する
...
| 調査条件を変えたい | 環境変数 `MYHOME_CONDITIONS` |
