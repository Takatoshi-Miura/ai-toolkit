---
name: cloud-am6-line-recommender
description: Claude Code のクラウドルーチンから毎日6時に呼び出され、環境変数の調査条件に基づきマイホーム（マンション）候補を1件提案してLINEグループに通知する。クラウド実行専用スキル（ローカルのスケジュールタスクでは使わない）。「LINE 通知」「マイホーム提案」などのキーワードが含まれる場合に使用。
allowed-tools: Read, Write, Bash, WebSearch, TodoWrite
user-invocable: true
disable-model-invocation: true
---

# cloud-am6-line-recommender

Claude Code のクラウドルーチンから呼び出される想定で、毎日6時の実行時にマイホーム（マンション）候補を1件調査し、LINE Messaging API でグループに通知する。設定・履歴・出力はすべてスキルディレクトリ内に内包する自己完結構成を取る。

## 役割

クラウドルーチンから渡された調査条件（環境変数）をもとに、マイホーム（マンション）候補を1件調査・選定し、LINE グループに通知する。
...
- スキルディレクトリ（`skills/cloud-am6-line-recommender/`）直下の references/CONFIG.md・references/history.md・output/ を読み書きする
...
- 本スキルはクラウドルーチン専用。時刻判定は行わない（ルーチン側で6時に発火済みの前提）
...
- `MYHOME_CONDITIONS`: 調査条件（JSON文字列）
...
`MYHOME_CONDITIONS` の調査条件に合う候補を WebSearch で 5 件ほど列挙する。

**調査範囲についての注意：**

- 除外対象は references/history.md に記載済みの**個別物件**のみ。過去に「候補なし」と判定された**エリア・駅**を除外してはならない。不動産在庫は日々入れ替わるため、過去に候補が出なかったエリアでも今回は新着物件が出ている可能性がある
- `MYHOME_CONDITIONS` の対象路線（湘南新宿ライン・宇都宮線・高崎線・京浜東北線）沿線の駅を広くローテーションで調査する。特定の駅・エリアに偏らず、直近の実行で調査していない駅を優先的に確認する
- references/history.md のエラー記録（各行の備考欄）を確認し、直近で重点的に調べたエリアと、しばらく調べていないエリアを把握したうえで、後者を優先的に調査する
...
Phase 1-2 で読み込んだ references/history.md の行に含まれる候補をすべて除外する。`MYHOME_CONDITIONS` の「避けたいもの」に合致するものも除外する。
...
```bash
python3 skills/cloud-am6-line-recommender/scripts/send_line.py "<本文>" "$LINE_GROUP_ID"
```
...
- **送信成功時**: `| YYYY-MM-DD | myhome | {選定タイトル/物件名} | {URLがあれば記載、なければ -} | - |`
- **送信失敗時**: `| YYYY-MM-DD | myhome | - | - | ERROR: {エラー内容} |`

### 5-2. output への保存

送信成功時のみ、送信した本文をそのまま `output/<YYYY-MM-DD>_myhome.md` に保存する（送信失敗時は保存しない）。
...
```bash
git add skills/cloud-am6-line-recommender/references/history.md skills/cloud-am6-line-recommender/output/
git commit -m "chore: cloud-am6-line-recommender history 更新 ($(date +%Y-%m-%d))"
git push origin HEAD

gh pr create --title "chore: cloud-am6-line-recommender history 更新 ($(date +%Y-%m-%d))" \
...
| エラー | 対応 |
|-------|------|
| `LINE_CHANNEL_ACCESS_TOKEN` / `LINE_GROUP_ID` / `MYHOME_CONDITIONS` が未設定 | クラウドルーチンの環境変数設定を確認する。references/SETUP.md を参照 |
...
- 最終的に「送信成功: myhome - {選定内容}」または「送信失敗: {エラー内容}」を報告する
...
- 調査条件を変更したい場合は、クラウドルーチンの環境変数 `MYHOME_CONDITIONS` を更新する（ファイル編集は不要）
